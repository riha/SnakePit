...
*/
(function(g,m){var p;
if(typeof g!="function"){throw"SignalR: jQuery not found. Please ensure jQuery is referenced before the SignalR.js file."
}if(!m.JSON){throw"SignalR: No JSON parser found. Please ensure json2.js is referenced before the SignalR.js file if you need to support clients without native JSON parsing support, e.g. IE<8."
}var d,j,k={onStart:"onStart",onStarting:"onStarting",onSending:"onSending",onReceived:"onReceived",onError:"onError",onReconnect:"onReconnect",onStateChanged:"onStateChanged",onDisconnect:"onDisconnect"},c=function(f,e){if(e!==!1){var h;
typeof m.console!="undefined"&&(h="["+(new Date).toTimeString()+"] SignalR: "+f,m.console.debug?m.console.debug(h):m.console.log&&m.console.log(h))
}},l=function(e){var f;
return(e=g.trim(e),e.indexOf("http")!==0)?!1:(f=m.document.createElement("a"),f.href=e,f.protocol+f.host!==m.location.protocol+m.location.host)
},b=function(f,e,h){return e===f.state?(g(f).trigger(k.onStateChanged,[{oldState:f.state,newState:h}]),f.state=h,!0):!1
},a=function(e){return e.state===d.connectionState.disconnected
};
d=function(e,h,f){return new d.fn.init(e,h,f)
},d.connectionState={connecting:0,connected:1,reconnecting:2,disconnected:4},d.fn=d.prototype={init:function(f,h,e){this.url=f,this.qs=h,typeof e=="boolean"&&(this.logging=e)
},ajaxDataType:"json",logging:!1,state:d.connectionState.disconnected,reconnectDelay:2000,start:function(t,n){var s=this,q={transport:"auto",jsonp:!1},f,r=g.Deferred(),i=m.document.createElement("a");
return b(s,d.connectionState.disconnected,d.connectionState.connecting)===!1?(r.resolve(s),r.promise()):(g.type(t)==="function"?n=t:g.type(t)==="object"&&(g.extend(q,t),g.type(q.callback)==="function"&&(n=q.callback)),i.href=s.url,i.protocol&&i.protocol!==":"?(s.protocol=i.protocol,s.host=i.host,s.baseUrl=i.protocol+"//"+i.host):(s.protocol=m.document.location.protocol,s.host=m.document.location.host,s.baseUrl=s.protocol+"//"+s.host),s.wsProtocol=s.protocol==="https:"?"wss://":"ws://",l(s.url)&&(s.log("Auto detected cross domain url."),q.transport==="auto"&&(q.jsonp||(q.jsonp=!g.support.cors,q.jsonp&&s.log("Using jsonp because this browser doesn't support CORS")),q.transport=q.jsonp===!0?"longPolling":["webSockets","longPolling"])),s.ajaxDataType=q.jsonp?"jsonp":"json",g(s).bind(k.onStart,function(){g.type(n)==="function"&&n.call(s),r.resolve(s)
}),f=function(x,o){if(o=o||0,o>=x.length){s.transport||r.reject("SignalR: No transport could be initialized successfully. Try specifying a different transport or none at all for auto initialization.");
return
}var w=x[o],v=g.type(w)==="object"?w:d.transports[w];
v.start(s,function(){s.transport=v,b(s,d.connectionState.connecting,d.connectionState.connected),g(s).trigger(k.onStart),g(m).unload(function(){s.stop(!1)
})
},function(){f(x,o+1)
})
},m.setTimeout(function(){var e=s.url+"/negotiate";
s.log("Negotiating with '"+e+"'."),g.ajax({url:e,global:!1,cache:!1,type:"GET",data:{},dataType:s.ajaxDataType,error:function(h){g(s).trigger(k.onError,[h.responseText]),r.reject("SignalR: Error during negotiation request: "+h.responseText),s.stop()
},success:function(o){if(s.appRelativeUrl=o.Url,s.id=o.ConnectionId,s.webSocketServerUrl=o.WebSocketServerUrl,!o.ProtocolVersion||o.ProtocolVersion!=="1.0"){g(s).trigger(k.onError,"SignalR: Incompatible protocol version."),r.reject("SignalR: Incompatible protocol version.");
return
}g(s).trigger(k.onStarting);
var h=[],v=[];
g.each(d.transports,function(u){if(u==="webSockets"&&!o.TryWebSockets){return !0
}v.push(u)
}),g.isArray(q.transport)?g.each(q.transport,function(){var u=this;
(g.type(u)==="object"||g.type(u)==="string"&&g.inArray(""+u,v)>=0)&&h.push(g.type(u)==="string"?""+u:u)
}):g.type(q.transport)==="object"||g.inArray(q.transport,v)>=0?h.push(q.transport):h=v,f(h)
}})
},0),r.promise())
},starting:function(f){var e=this,h=g(e);
return h.bind(k.onStarting,function(){f.call(e),h.unbind(k.onStarting)
}),e
},send:function(e){var f=this;
if(f.state===d.connectionState.disconnected){throw"SignalR: Connection must be started before data can be sent. Call .start() before .send()"
}if(f.state===d.connectionState.connecting){throw"SignalR: Connection has not been fully initialized. Use .start().done() or .start().fail() to run logic after the connection has started."
}return f.transport.send(f,e),f
},sending:function(f){var e=this;
return g(e).bind(k.onSending,function(){f.call(e)
}),e
},received:function(f){var e=this;
return g(e).bind(k.onReceived,function(h,i){f.call(e,i)
}),e
},stateChanged:function(f){var e=this;
return g(e).bind(k.onStateChanged,function(h,i){f.call(e,i)
}),e
},error:function(f){var e=this;
return g(e).bind(k.onError,function(h,i){f.call(e,i)
}),e
},disconnected:function(f){var e=this;
return g(e).bind(k.onDisconnect,function(){f.call(e)
}),e
},reconnected:function(f){var e=this;
return g(e).bind(k.onReconnect,function(){f.call(e)
}),e
},stop:function(e){var f=this;
if(f.state!==d.connectionState.disconnected){try{f.transport&&(f.transport.abort(f,e),f.transport.stop(f),f.transport=null),g(f).trigger(k.onDisconnect),delete f.messageId,delete f.groups
}finally{b(f,f.state,d.connectionState.disconnected)
}return f
}},log:function(e){c(e,this.logging)
}},d.fn.init.prototype=d.fn,p={addQs:function(e,f){return f.qs?typeof f.qs=="object"?e+"&"+g.param(f.qs):typeof f.qs=="string"?e+"&"+f.qs:e+"&"+m.escape(f.qs.toString()):e
},getUrl:function(t,s,w,x){var v=s==="webSockets"?"":t.baseUrl,q=v+t.appRelativeUrl,h="transport="+s+"&connectionId="+m.escape(t.id);
return t.data&&(h+="&connectionData="+m.escape(t.data)),w?(x&&(q=q+"/reconnect"),t.messageId&&(h+="&messageId="+t.messageId),t.groups&&(h+="&groups="+m.escape(JSON.stringify(t.groups)))):q=q+"/connect",q+="?"+h,q=this.addQs(q,t),q+="&tid="+Math.floor(Math.random()*11)
},ajaxSend:function(h,n){var e=h.url+"/send?transport="+h.transport.name+"&connectionId="+m.escape(h.id);
e=this.addQs(e,h),g.ajax({url:e,global:!1,type:"POST",dataType:h.ajaxDataType,data:{data:n},success:function(f){f&&g(h).trigger(k.onReceived,[f])
},error:function(f,i){i!=="abort"&&(i!=="parsererror"||h.ajaxDataType!=="jsonp")&&g(h).trigger(k.onError,[f])
}})
},ajaxAbort:function(e,f){if(typeof e.transport!="undefined"){f=typeof f=="undefined"?!0:f;
var h=e.url+"/abort?transport="+e.transport.name+"&connectionId="+m.escape(e.id);
h=this.addQs(h,e),g.ajax({url:h,async:f,timeout:1000,global:!1,type:"POST",dataType:e.ajaxDataType,data:{}}),e.log("Fired ajax abort async = "+f)
}},processMessages:function(f,e){var h=g(f);
if(e){if(e.Disconnect){f.log("Disconnect command received from server"),f.stop();
return
}e.Messages&&g.each(e.Messages,function(){try{h.trigger(k.onReceived,[this])
}catch(n){f.log("Error raising received "+n),g(f).trigger(k.onError,[n])
}}),e.MessageId&&(f.messageId=e.MessageId),e.TransportData&&(f.groups=e.TransportData.Groups)
}},foreverFrame:{count:0,connections:{}}},d.transports={webSockets:{name:"webSockets",send:function(e,f){e.socket.send(f)
},start:function(n,t,u){var q,f=!1,r=this,i=!t,x,w=g(n);
if(m.MozWebSocket&&(m.WebSocket=m.MozWebSocket),!m.WebSocket){u();
return
}n.socket||(q=n.webSocketServerUrl?n.webSocketServerUrl:n.wsProtocol+n.host,g(n).trigger(k.onSending),q+=p.getUrl(n,this.name,i),n.log("Connecting to websocket endpoint '"+q+"'"),n.socket=new m.WebSocket(q),n.socket.onopen=function(){f=!0,n.log("Websocket opened"),t?t():b(n,d.connectionState.reconnecting,d.connectionState.connected)===!0&&w.trigger(k.onReconnect)
},n.socket.onclose=function(e){if(f){typeof e.wasClean!="undefined"&&e.wasClean===!1?(g(n).trigger(k.onError,[e.reason]),n.log("Unclean disconnect from websocket."+e.reason)):n.log("Websocket closed")
}else{u?u():i&&r.reconnect(n);
return
}r.reconnect(n)
},n.socket.onmessage=function(h){var e=m.JSON.parse(h.data),s;
e&&(s=g(n),e.Messages?p.processMessages(n,e):s.trigger(k.onReceived,[e]))
})
},reconnect:function(e){this.stop(e),(e.state===d.connectionState.reconnecting||b(e,d.connectionState.connected,d.connectionState.reconnecting)===!0)&&(e.log("Websocket reconnecting"),this.start(e))
},stop:function(e){e.socket!==null&&(e.log("Closing the Websocket"),e.socket.close(),e.socket=null)
},abort:function(){}},serverSentEvents:{name:"serverSentEvents",timeOut:3000,start:function(n,t,w){var i=this,f=!1,r=g(n),q=!t,z,x;
if(n.eventSource&&(n.log("The connection already has an event source. Stopping it."),n.stop()),!m.EventSource){w&&(n.log("This browser doesn't support SSE."),w());
return
}r.trigger(k.onSending),z=p.getUrl(n,this.name,q);
try{n.log("Attempting to connect to SSE endpoint '"+z+"'"),n.eventSource=new m.EventSource(z)
}catch(u){n.log("EventSource failed trying to connect with error "+u.Message),w?w():(r.trigger(k.onError,[u]),q&&i.reconnect(n));
return
}x=m.setTimeout(function(){f===!1&&(n.log("EventSource timed out trying to connect"),n.log("EventSource readyState: "+n.eventSource.readyState),q||i.stop(n),q?n.eventSource.readyState!==m.EventSource.CONNECTING&&n.eventSource.readyState!==m.EventSource.OPEN&&i.reconnect(n):w&&w())
},i.timeOut),n.eventSource.addEventListener("open",function(){n.log("EventSource connected"),x&&m.clearTimeout(x),f===!1&&(f=!0,t&&t(),q&&b(n,d.connectionState.reconnecting,d.connectionState.connected)===!0&&r.trigger(k.onReconnect))
},!1),n.eventSource.addEventListener("message",function(e){e.data!=="initialized"&&p.processMessages(n,m.JSON.parse(e.data))
},!1),n.eventSource.addEventListener("error",function(e){if(!f){w&&w();
return
}n.log("EventSource readyState: "+n.eventSource.readyState),e.eventPhase===m.EventSource.CLOSED?(n.log("EventSource reconnecting due to the server connection ending"),i.reconnect(n)):(n.log("EventSource error"),r.trigger(k.onError))
},!1)
},reconnect:function(e){var f=this;
m.setTimeout(function(){f.stop(e),(e.state===d.connectionState.reconnecting||b(e,d.connectionState.connected,d.connectionState.reconnecting)===!0)&&(e.log("EventSource reconnecting"),f.start(e))
},e.reconnectDelay)
},send:function(e,f){p.ajaxSend(e,f)
},stop:function(e){e&&e.eventSource&&(e.log("EventSource calling close()"),e.eventSource.close(),e.eventSource=null,delete e.eventSource)
},abort:function(e,f){p.ajaxAbort(e,f)
}},foreverFrame:{name:"foreverFrame",timeOut:3000,start:function(u,r,q){var x=this,t=p.foreverFrame.count+=1,n,v,w=g("<iframe data-signalr-connection-id='"+u.id+"' style='position:absolute;top:0;left:0;width:0;height:0;visibility:hidden;'></iframe>");
if(m.EventSource){q&&(u.log("This brower supports SSE, skipping Forever Frame."),q());
return
}g(u).trigger(k.onSending),n=p.getUrl(u,this.name),n+="&frameId="+t,w.prop("src",n),p.foreverFrame.connections[t]=u,u.log("Binding to iframe's readystatechange event."),w.bind("readystatechange",function(){g.inArray(this.readyState,["loaded","complete"])>=0&&(u.log("Forever frame iframe readyState changed to "+this.readyState+", reconnecting"),x.reconnect(u))
}),u.frame=w[0],u.frameId=t,r&&(u.onSuccess=r),g("body").append(w),v=m.setTimeout(function(){u.onSuccess&&(u.log("Failed to connect using forever frame source, it timed out after "+x.timeOut+"ms."),x.stop(u),q&&q())
},x.timeOut)
},reconnect:function(e){var f=this;
m.setTimeout(function(){if(e.frame&&(e.state===d.connectionState.reconnecting||b(e,d.connectionState.connected,d.connectionState.reconnecting)===!0)){var h=e.frame,i=p.getUrl(e,f.name,!0)+"&frameId="+e.frameId;
e.log("Upating iframe src to '"+i+"'."),h.src=i
}},e.reconnectDelay)
},send:function(e,f){p.ajaxSend(e,f)
},receive:function(h,e){var f;
p.processMessages(h,e),h.frameMessageCount=(h.frameMessageCount||0)+1,h.frameMessageCount>50&&(h.frameMessageCount=0,f=h.frame.contentWindow||h.frame.contentDocument,f&&f.document&&g("body",f.document).empty())
},stop:function(f){var e=null;
f.frame&&(f.frame.stop?f.frame.stop():(e=f.frame.contentWindow||f.frame.contentDocument,e.document&&e.document.execCommand&&e.document.execCommand("Stop")),g(f.frame).remove(),delete p.foreverFrame.connections[f.frameId],f.frame=null,f.frameId=null,delete f.frame,delete f.frameId,f.log("Stopping forever frame"))
},abort:function(e,f){p.ajaxAbort(e,f)
},getConnection:function(e){return p.foreverFrame.connections[e]
},started:function(e){e.onSuccess?(e.onSuccess(),e.onSuccess=null,delete e.onSuccess):b(e,d.connectionState.reconnecting,d.connectionState.connected)===!0&&g(e).trigger(k.onReconnect)
}},longPolling:{name:"longPolling",reconnectDelay:3000,start:function(h,i){var f=this,e=!1;
h.pollXhr&&(h.log("Polling xhr requests already exists, aborting."),h.stop()),h.messageId=null,m.setTimeout(function(){(function n(o,u){g(o).trigger(k.onSending);
var r=o.messageId,s=r===null,q=!s,x=p.getUrl(o,f.name,q,u),t=null,z=!1;
(q!==!0||u!==!0||h.state===d.connectionState.reconnecting||b(h,d.connectionState.connected,d.connectionState.reconnecting)!==!1)&&(h.log("Attempting to connect to '"+x+"' using longPolling."),o.pollXhr=g.ajax({url:x,global:!1,type:"GET",dataType:h.ajaxDataType,success:function(v){var A=0,y=!1;
(e==!1&&(i(),e=!0),u===!0&&z===!1&&(h.log("Raising the reconnect event"),b(h,d.connectionState.reconnecting,d.connectionState.connected)===!0&&(g(o).trigger(k.onReconnect),z=!0)),p.processMessages(o,v),v&&v.TransportData&&g.type(v.TransportData.LongPollDelay)==="number"&&(A=v.TransportData.LongPollDelay),v&&v.TimedOut&&(y=v.TimedOut),v&&v.Disconnect)||a(o)!==!0&&(A>0?m.setTimeout(function(){n(o,y)
},A):n(o,y))
},error:function(v,w){if(w==="abort"){h.log("Aborted xhr requst.");
return
}h.log("An error occurred using longPolling. Status = "+w+". "+v.responseText),t&&clearTimeout(t),g(o).trigger(k.onError,[v.responseText]),m.setTimeout(function(){a(o)===!1&&n(o,!0)
},h.reconnectDelay)
}}),u===!0&&(t=m.setTimeout(function(){z===!1&&b(h,d.connectionState.reconnecting,d.connectionState.connected)===!0&&(g(o).trigger(k.onReconnect),z=!0)
},f.reconnectDelay)))
})(h),m.setTimeout(function(){e===!1&&(i(),e=!0)
},150)
},250)
},send:function(e,f){p.ajaxSend(e,f)
},stop:function(e){e.pollXhr&&(e.pollXhr.abort(),e.pollXhr=null,delete e.pollXhr)
},abort:function(e,f){p.ajaxAbort(e,f)
}}},d.noConflict=function(){return g.connection===d&&(g.connection=j),d
},g.connection&&(j=g.connection),g.connection=g.signalR=d
})(window.jQuery,window);
var Shared=(function(){if(typeof console==="undefined"){console={log:function(){}}
}window.requestAnimFrame=(function(){return window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(a,b){return window.setTimeout(a,1000/60)
}
})();
...
window.Directions={Stopped:{x:0,y:0},Up:{x:0,y:-1},Down:{x:0,y:1},Left:{x:-1,y:0},Right:{x:1,y:0}};
function roundRect(a,g,h,f,c,d,b,e){if(typeof e=="undefined"){e=true
}if(typeof d==="undefined"){d=5
}a.beginPath();
a.moveTo(g+d,h);
a.lineTo(g+f-d,h);
a.quadraticCurveTo(g+f,h,g+f,h+d);
a.lineTo(g+f,h+c-d);
a.quadraticCurveTo(g+f,h+c,g+f-d,h+c);
a.lineTo(g+d,h+c);
a.quadraticCurveTo(g,h+c,g,h+c-d);
a.lineTo(g,h+d);
a.quadraticCurveTo(g,h,g+d,h);
a.closePath();
if(e){a.stroke()
}if(b){a.fill()
...
a.moveTo(100,0);
a.lineTo(350,0);
a.quadraticCurveTo(250,250,200,500);
a.lineTo(30,500);
a.quadraticCurveTo(80,80,100,0);
...
roundRect(b,-8,-4,j*2,j,3,true,false);
roundRect(b,-4,-8,j,j*2,3,true,false);
b.fillStyle="#AACC99";
b.strokeStyle="#000";
roundRect(b,-2.5,-2.5,5,5,2,true,false);
b.restore()
}i()
};
var Point=function(b,c){var a=this;
a.x=b;
a.y=c;
a.toFixed=function(){return new Point(a.x*20,a.y*20)
}
};
var Snake=function(h,b,c){var e=this;
e.currentHeadPoint=h;
e.parts=[h];
a(14);
function a(k){console.log("Adding number of parts to snake",k);
var l=h.x;
for(var j=0;
j<k;
j++){l-=1;
e.parts.push(new Point(l,h.y))
}}function f(i){e.currentHeadPoint.x+=i.x*1;
e.currentHeadPoint.y+=i.y*1
}e.grow=function(){a(1)
};
...
roundRect(b,e.parts[0].toFixed().x,e.parts[0].toFixed().y,18,18,7,true,false);
b.restore();
d=Date.now();
return true
}return false
